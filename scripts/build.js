...
// 1. Read and index all category files from question-bank/
...
qbFiles.forEach(file => {
  ...
  try {
    ...
    if (Array.isArray(questions)) {
      ...
      const easy = questions.filter(q => (q.difficulty || "").toLowerCase() === "easy" && (q.status || "active") === "active").length;
      const medium = questions.filter(q => (q.difficulty || "").toLowerCase() === "medium" && (q.status || "active") === "active").length;
      const hard = questions.filter(q => (q.difficulty || "").toLowerCase() === "hard" && (q.status || "active") === "active").length;
      const activeTotal = easy + medium + hard;
...
console.log(`✓ Indexed ${totalActiveQuestions} active questions across ${qbFiles.length} category files`);
...
// Extract a compact baseline pack (up to 6 questions per category: 2 Easy, 2 Medium, 2 Hard)
// This ensures instant gameplay on Blogger while full 2,285+ question pools are pulled from Git on demand
const basePack = [];
...
qbFiles.forEach(file => {
  ...
  try {
    const questions = JSON.parse(fs.readFileSync(filePath, "utf8"));
    if (Array.isArray(questions)) {
      const easy = questions.filter(q => (q.difficulty || "").toLowerCase() === "easy" && (q.status || "active") === "active").slice(0, 2);
      const med = questions.filter(q => (q.difficulty || "").toLowerCase() === "medium" && (q.status || "active") === "active").slice(0, 2);
      const hard = questions.filter(q => (q.difficulty || "").toLowerCase() === "hard" && (q.status || "active") === "active").slice(0, 2);
      basePack.push(...easy, ...med, ...hard);
    }
  } catch (e) {}
});

console.log(`✓ Generated compact baseline pack (${basePack.length} questions, ~35 KB) for instant offline/Blogger fallback`);

// 3. Write lightweight src/questions.js (Index for instant count + baseline fallback, full pools in Git)
const questionsJsContent = `/**
 * WKQUIZ QUESTION BANK METADATA INDEX & BASELINE PACK
 * Auto-generated by scripts/build.js
 * Total Questions in Git Repository: ${totalActiveQuestions}
...
const WKQUIZ_BASE_PACK = ${JSON.stringify(basePack, null, 2)};
...
console.log("✓ Updated src/questions.js (Lightweight Git Index & Baseline Pack)");
